...
export default function FinanceTab({ car, totalGop, totalChiPhi }: FinanceTabProps) {
    return (
        <div className="p-4 space-y-4">
             {/* Main Financial Card */}
             <div className="bg-linear-to-br from-slate-900 to-black text-white p-6 rounded-3xl shadow-xl shadow-slate-200 dark:shadow-none relative overflow-hidden">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
                <div className="px-5 py-4 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Danh sách góp vốn</span>
                    <Link href={`/cars/${car.id}/investors/new`} className="text-violet-600 dark:text-violet-400 text-xs font-bold hover:bg-violet-50 dark:hover:bg-violet-900/20 px-2 py-1 rounded-lg transition-colors">
                        + Thêm
                    </Link>
                </div>
                <div className="divide-y divide-slate-50 dark:divide-slate-800">
                    {car.gopVon.map((inv) => (
                        <div key={inv.id} className="p-4 flex justify-between items-center">
                            <div className="flex items-center">
                                <div className="w-8 h-8 rounded-full bg-violet-100 dark:bg-violet-900/20 text-violet-600 dark:text-violet-400 flex items-center justify-center font-bold text-xs mr-3">
                                    {inv.nguoiGop.charAt(0)}
                                </div>
                                <div>
                                    <p className="font-bold text-sm text-slate-900 dark:text-white">{inv.nguoiGop}</p>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{inv.tyLeGop}% cổ phần</p>
                                </div>
                            </div>
                    ...
                    {car.gopVon.length === 0 && (
                        <div className="p-8 text-center text-slate-400 text-xs italic">Chưa có nhà đầu tư nào.</div>
                    )}
                </div>
            </div>
        </div>
